...
func newGenerateCmd(output io.Writer) *cobra.Command {
	...
	f.StringVar(&generate.caPEMFileIn, "caPEMFileIn", "", "full path to the root cert to be loaded")
	f.StringVar(&generate.caKeyPEMFileIn, "caKeyPEMFileIn", "", "full path to the root cert key to be loaded")
