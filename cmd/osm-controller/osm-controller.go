...
var (
	...
	port                = flags.Int("port", constants.OSMControllerPort, "Aggregated Discovery NamespacedService port number.")
	...
	vaultRole     = flags.String("vault-role", "openservicemesh", "Name of the Vault role dedicated to Open NamespacedService Mesh")
)
...
func main() {
	...
	log.Info().Msgf("NamespacedService certificates will be valid for %+v", getServiceCertValidityPeriod())
