...
type installCmd struct {
	...
	// Toggle to enable jetstack/cert-manager
...
func newInstallCmd(config *helm.Configuration, out io.Writer) *cobra.Command {
	...
	f.BoolVar(&inst.enableCertManager, "enable-cert-manager", true, "Enable jetstack cert-manager deployment")
...
func (i *installCmd) resolveValues() (map[string]interface{}, error) {
	...
	valuesConfig := []string{
		...
		fmt.Sprintf("OpenServiceMesh.enableCertManager=%t", i.enableCertManager),
