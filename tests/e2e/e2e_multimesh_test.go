...
// This tests connectivity between two Service Meshes within one cluster.
// In theory, this could be extended to two SMs in different clusters
var _ = OSMDescribe("Testing HTTP Traffic from one mesh to another",
	OSMDescribeInfo{
		Tier:   1,
		Bucket: 7,
	},
	func() {
		Context("Test traffic flowing from client to server with a Kubernetes Service for the Source: HTTP", func() {
			...
			testMultimesh(withSourceKubernetesService)
		})
...
func testMultimesh(withSourceKubernetesService bool) {
	...
	It("Tests HTTP traffic for client pod -> server pod", func() {
		...
		// Install client mesh - slightly hacky way
		...
		Expect(Td.AddNsToSpecificMesh(true, sourceMeshName, sourceName)).To(Succeed())

		// Create source/client pod
		srcPod := setupMultimeshSource(sourceName, withSourceKubernetesService)

		// TODO - Add MultiClusterDefinition with both service accounts?

		// TODO - Update to Multimesh?
		...
		// TODO - Also add to sourceMeshName?
		...
		clientToServer := HTTPRequestDef{
			...
			Destination: fmt.Sprintf("%s.%s.svc.cluster.%s", dstSvc.Name, dstSvc.Namespace, "local"), // TODO - dstSvc.ClusterDomain?
		}
		...
		// TODO test deleting in only default mesh and only in sourceMesh and deleting both for sanity check?
...
func setupMultimeshSource(sourceName string, withKubernetesService bool) *v1.Pod {
