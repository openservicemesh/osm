...
func TestRDSNewResponseWithTrafficSplit(t *testing.T) {
	...
	allTrafficTargets := []*smiAccess.TrafficTarget{&tests.TrafficTarget, &tests.BookstoreV2TrafficTarget}
	allTrafficSplits := []*smiSplit.TrafficSplit{&tests.TrafficSplit}
	...
	provider.EXPECT().ListTrafficTargets().Return(allTrafficTargets).AnyTimes()
	provider.EXPECT().ListTrafficSplits().Return(allTrafficSplits).AnyTimes()
