...
```bash
$ kubectl apply -f demo/manifests/apps/
```
...
Postion the windows so that you can see all four at the same time. The header at the top of the webpage indicates the application and version. The window for `bookstore-v2` should not be working. Keep it open anyway for the Traffic Split Configuration section later in the demo.
...
```bash
kubectl apply -f demo/manifests/access/
```
...
```bash
$ kubectl apply -f demo/manifests/bookstore-services/
```
...
```bash
$ kubectl apply -f demo/manifests/split/traffic-split.yaml
```

### Deploy v2 of Bookstore
```
$ kubectl apply -f demo/manifests/bookstore-v2/
```
