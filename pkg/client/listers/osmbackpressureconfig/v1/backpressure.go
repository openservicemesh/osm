...
type BackpressureLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Backpressure, err error)
...
// List lists all Backpressures in the indexer.
func (s *backpressureLister) List(selector labels.Selector) (ret []*v1.Backpressure, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Backpressure))
	})
	return ret, err
}
...
type BackpressureNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.Backpressure, error)
...
// List lists all Backpressures in the indexer for a given namespace.
func (s backpressureNamespaceLister) List(selector labels.Selector) (ret []*v1.Backpressure, err error) {
...
// Get retrieves the Backpressure from the indexer for a given namespace and name.
func (s backpressureNamespaceLister) Get(name string) (*v1.Backpressure, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("backpressure"), name)
	}
	return obj.(*v1.Backpressure), nil
}
