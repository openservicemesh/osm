...
func newFakeMeshCatalogForRoutes(t *testing.T, permissive bool) *MeshCatalog {
	...
	mockMeshSpec.EXPECT().GetAnnouncementsChannel().Return(testChan).AnyTimes()
	...
	mockConfigurator.EXPECT().IsPermissiveTrafficPolicyMode().Return(permissive).AnyTimes()
