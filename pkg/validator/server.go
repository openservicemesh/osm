...
func NewValidatingWebhook(ctx context.Context, webhookConfigName, osmNamespace, osmVersion, meshName string, enableReconciler, validateTrafficTarget bool, certManager *certificate.Manager, broker *messaging.Broker, kubeClient kubernetes.Interface, policyClient policy.Controller) error {
	kv := &policyValidator{
	...
	srv, err := webhook.NewServer(ValidatorWebhookSvc, osmNamespace, constants.ValidatorWebhookPort, certManager, broker, map[string]http.HandlerFunc{
