...
var _ = Describe("Test XDS certificate tooling", func() {
	...
	Context("Test GetServicesForProxy()", func() {
		It("works as expected", func() {
			...
			proxy := envoy.NewProxy(certCommonName, certSerialNumber, nil, nil)
			meshServices := proxy.GetServices()
	...
	Context("Test getServiceFromCertificate()", func() {
		It("works as expected", func() {
			...
			certSerialNumber := certificate.SerialNumber("123456")
			newProxy := envoy.NewProxy(podCN, certSerialNumber, nil, nil)
			meshServices := newProxy.GetServices()
	...
	Context("Test GetPodFromCertificate()", func() {
		It("works as expected", func() {
			...
			actualPod, err := mockKubeController.GetPodFromCertificate(newCN)
			Expect(err).ToNot(HaveOccurred())
