...
func TestNewResponse(t *testing.T) {
	...
	fakeVersionedClient := testclientVersioned.NewSimpleClientset()
	...
	cfg := configurator.NewConfiguratorWithCRDClient(fakeVersionedClient, stop, namespace, "-the-config-map-name-")
	certManager := tresor.NewFakeCertManager(cfg)
	meshCatalog := catalog.NewFakeMeshCatalog(fakeKubeClient, fakeVersionedClient)
