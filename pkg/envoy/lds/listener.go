...
func newOutboundListener(catalog catalog.MeshCataloger, cfg configurator.Configurator, downstreamIdentity service.K8sServiceAccount) (*xds_listener.Listener, error) {
	serviceFilterChains, err := getOutboundFilterChains(catalog, cfg, downstreamIdentity)
	if err != nil {
...
func getOutboundFilterChains(catalog catalog.MeshCataloger, cfg configurator.Configurator, downstreamIdentity service.K8sServiceAccount) ([]*xds_listener.FilterChain, error) {
	...
	outboundSvc, err := catalog.ListAllowedOutboundServices(downstreamIdentity)
	if err != nil {
		log.Error().Err(err).Msgf("Error getting allowed outbound services for %s", downstreamIdentity.String())
		return nil, err
	}
