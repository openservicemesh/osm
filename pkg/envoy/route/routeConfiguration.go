...
func UpdateRouteConfiguration(rotesPerHost map[string]map[string]trafficpolicy.RouteWeightedClusters, routeConfig *v2.RouteConfiguration, isSourceConfig bool, isDestinationConfig bool) {
	var isLocalCluster bool
	var virtualHostPrefix string

	if isSourceConfig {
		log.Trace().Msgf("[RDS] Updating OutboundRouteConfiguration for policy %v", rotesPerHost)
		isLocalCluster = false
		virtualHostPrefix = outboundVirtualHost
	} else if isDestinationConfig {
		log.Trace().Msgf("[RDS] Updating InboundRouteConfiguration for policy %v", rotesPerHost)
		isLocalCluster = true
		virtualHostPrefix = inboundVirtualHost
	}
	for host, routePolicyWeightedClustersMap := range rotesPerHost {
