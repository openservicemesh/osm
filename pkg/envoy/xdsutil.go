...
func GetDownstreamTLSContext(upstreamIdentity identity.ServiceIdentity, mTLS bool, sidecarSpec configv1alpha2.SidecarSpec) *xds_auth.DownstreamTlsContext {
	upstreamSDSCert := secrets.GetSDSServiceCertForIdentity(upstreamIdentity)
	...
	if mTLS {
		...
		downstreamPeerValidationSDSCert = secrets.GetSDSInboundRootCertForIdentity(upstreamIdentity)
	}
...
func GetUpstreamTLSContext(downstreamIdentity identity.ServiceIdentity, upstreamSvc service.MeshService, sidecarSpec configv1alpha2.SidecarSpec) *xds_auth.UpstreamTlsContext {
	downstreamSDSCert := secrets.GetSDSServiceCertForIdentity(downstreamIdentity)

	upstreamPeerValidationSDSCert := secrets.GetSDSOutboundRootCertForService(upstreamSvc)
