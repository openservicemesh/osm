...
const (
	...
	apiGroup = "remoteservice.openservicemesh.io"

	// remoteServiceSourceKindSvcAccount is the ServiceAccount kind for a source defined in Egress remoteservice
	remoteServiceSourceKindSvcAccount = "ServiceAccount"
)

// NewConfigController returns a remoteservice.Controller interface related to functionality provided by the resources in the remoteservice.openservicemesh.io API group
...
// newConfigClient creates k8s clients for the resources in the remoteservice.openservicemesh.io API group
func newConfigClient(configClient configV1alpha1Client.Interface, kubeController kubernetes.Controller, stop chan struct{}) (client, error) {
	...
	informerCollection := informerCollection{
		remoteService: informerFactory.Config().V1alpha1().RemoteServices().Informer(),
	}
	...
	remoteServiceEventTypes := kubernetes.EventTypes{
		Add:    announcements.ConfigAdded,
		Update: announcements.ConfigUpdated,
		Delete: announcements.ConfigDeleted,
	}
	informerCollection.remoteService.AddEventHandler(kubernetes.GetKubernetesEventHandlers("Config", "Policy", shouldObserve, remoteServiceEventTypes))
