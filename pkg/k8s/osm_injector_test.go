...
func TestGetOSMInjectorPods(t *testing.T) {
	...
	tests := []struct {
		...
		{
			testName: "multiple pods (osm-injector pods and other pods) in multiple namespaces",
			pods: []*corev1.Pod{
				{
	...
	for _, test := range tests {
		t.Run(test.testName, func(t *testing.T) {
			...
			podList := GetOSMInjectorPods(fakeClientSet, testNamespace)
