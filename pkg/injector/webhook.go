...
func NewWebhook(config Config, kubeClient kubernetes.Interface, certManager certificate.Manager, meshCatalog catalog.MeshCataloger, Controller k8s.Controller, meshName, osmNamespace, webhookName string, stop <-chan struct{}, cfg configurator.Configurator) error {
...
func (wh *webhook) isNamespaceAllowed(namespace string) bool {
	...
	return wh.Controller.IsMonitoredNamespace(namespace)
}
