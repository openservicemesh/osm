...
`
	...
	Context("create envoy config", func() {
		It("creates envoy config", func() {
			actual, jsonConfig, err := getEnvoyConfigYAML(config)
			Expect(err).ToNot(HaveOccurred())

			Expect(string(actual)).To(Equal(expectedEnvoyConfig),
				fmt.Sprintf("Expected:\n%s\nActual:\n%s\n", expectedEnvoyConfig, string(actual)))

			expectedJSONConfig := []uint8(`{"admin":{"access_log_path":"/dev/stdout","address":{"socket_address":{"address":"0.0.0.0","port_value":"15000"}}},"dynamic_resources":{"ads_config":{"api_type":"GRPC","grpc_services":[{"envoy_grpc":{"cluster_name":"osm-controller"}}],"set_node_on_first_message_only":true,"transport_api_version":"V3"},"cds_config":{"ads":{},"resource_api_version":"V3"},"lds_config":{"ads":{},"resource_api_version":"V3"}},"static_resources":{"clusters":[{"connect_timeout":"0.25s","http2_protocol_options":{},"load_assignment":{"cluster_name":"osm-controller","endpoints":[{"lb_endpoints":[{"endpoint":{"address":{"socket_address":{"address":"osm-controller.b.svc.cluster.local","port_value":15128}}}}]}]},"name":"osm-controller","transport_socket":{"name":"envoy.transport_sockets.tls","typed_config":{"@type":"type.googleapis.com/envoy.extensions.transport_sockets.tls.v3.UpstreamTlsContext","common_tls_context":{"alpn_protocols":["h2"],"tls_certificates":[{"certificate_chain":{"inline_bytes":"eHg="},"private_key":{"inline_bytes":"eXk="}}],"tls_params":{"tls_maximum_protocol_version":"TLSv1_3","tls_minimum_protocol_version":"TLSv1_2"},"validation_context":{"trusted_ca":{"inline_bytes":"eHg="}}}}},"type":"LOGICAL_DNS"}]}}`)
			Expect(jsonConfig).To(Equal(expectedJSONConfig))
		})

		It("Creates bootstrap config for the Envoy proxy", func() {
			osmNamespace := "b"
			jsonConfig, err := getEnvoyBootstrapConfig(osmNamespace, cert)
			Expect(err).ToNot(HaveOccurred())
...
var _ = Describe("Test Envoy sidecar", func() {
	...
	Context("create Envoy sidecar", func() {
		It("creates correct Envoy sidecar spec", func() {
			...
			envoyConfig := "{}"
			actual := getEnvoySidecarContainerSpec(containerName, envoyImage, nodeID, clusterID, envoyConfig, mockConfigurator)
