...
type MultiClusterServiceLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.MultiClusterService, err error)
...
// List lists all MultiClusterServices in the indexer.
func (s *multiClusterServiceLister) List(selector labels.Selector) (ret []*v1alpha2.MultiClusterService, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.MultiClusterService))
	})
	return ret, err
}
...
type MultiClusterServiceNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.MultiClusterService, error)
...
// List lists all MultiClusterServices in the indexer for a given namespace.
func (s multiClusterServiceNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.MultiClusterService, err error) {
...
// Get retrieves the MultiClusterService from the indexer for a given namespace and name.
func (s multiClusterServiceNamespaceLister) Get(name string) (*v1alpha2.MultiClusterService, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("multiclusterservice"), name)
	}
	return obj.(*v1alpha2.MultiClusterService), nil
}
