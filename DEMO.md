...
## Prerequisites
This demo assumes:
1. You have cloned this repo on your workstation and have installed `go 1.13`: `which go && go version`
1. You have OpenSSL installed (LibreSSL 2.8.3 or later): `which openssl && openssl version`
1. You have installed `kubectl` on your workstation: `which kubectl && kubectl version`
1. Access to a Kubernetes cluster has been provisionad and the credentials are in `~/.kube/config` or the correct path is set as `$KUBECONFIG`:
   - The Azure Kubernetes Service is a fitting provider of a hosted Kubernetes service
   - [Install Azure CLI](https://docs.microsoft.com/azure/) ?????????????
...
1. You have installed Docker CLI on the workstation running the commands below.
...
1. You have authenticated with a container registry, which is accessible to both your workstation and your Kubernetes cluster. One such registry is the Azure Container Registry (ACR), which is used by the demo scripts in this repo:
   - [Install Azure CLI](https://docs.microsoft.com/azure/) ?????????????
   - Login to your Azure account: `az login`
...
1. From the root of this repository execute `./demo/run-demo.sh`. This script will
   - compile both the Enpdoint Discovery Service and the Secrets Discovery Service, create separate containers and push these to the workstation's default container registry (See `~/.docker/config.json`)
   ...
   - bootstrap Envoy configs (ConfigMaps) for the bookstore and bookbuyer services are also uploaded to the K8s cluster
