...
2. Find your Workspace ID and Primary key in your workspace under Advanced settings > Agents management. Update the `[OUTPUT]` section as follows:
   ```
   [OUTPUT]
         Name        azure
         Match       *
...
4. Once you run OSM with Fluent Bit enabled, logs will populate under the Logs > Custom Logs section in Log Analytics. There, you may run the following query to view most recent logs first:
    ```
    fluentbit_CL
...
2. Navigate to your instance in Azure Portal. Go to the Logs section. Run this query to ensure that logs are being picked up from Log Analytics:
    ```
    workspace("fluent-bit-log-poc").fluentbit_CL
    ```
