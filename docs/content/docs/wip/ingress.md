...
- The `networking/v1beta1` API must be used for the Kubernetes Ingress resource

## What is ingress? 

Ingress refers to providing external access to services inside the cluster, usually via HTTP/HTTPS. In kubernetes, Ingress consists of an Ingress API object and an Ingress Controller. The Kubernetes Ingress API manifest consists of declarations of how clients are routed to a service inside the cluster, and the Ingress controller executes the routing declarations specified in the manifest. 
